...
- Housing production is picking up
- Negative natural change will hamper growth
- No adjustments to the county
- Minor adjustments to UGBs to capture faster Redmond growth

---

# Deschutes at a Glance: 2020 Census

```{r echo = FALSE, message = FALSE, warning = FALSE}
library(tidyverse)
library(plotly)
focus_county <- "Deschutes"
model <- "model_41"
...
# Deschutes Population

.pulll-left[
Growth in the county has slowed with the COVID-19 pandemic but still remains an outlier amongst others in the state.
]
...
# Deschutes Compositional Changes

.pulll-left[
Migration has consistently been high in the county, however, natural change has been on the decline.
]
...
.pulll-left[
Births have increased over time but are expected to level off in the coming years with continued decline in fertility rates.
]
...
.pulll-left[
Deaths have been increasing, a pattern that is expected to continue as the population ages.
]
...
.pulll-left[
Substantial aging has occurred in the county over the past 30 years, and continue in the forecast period.
]
...
# Deschutes Race and Ethnicity

.pulll-left[
The population is expected to continue to diversify with all groups expected to grow in the forecast period.
]
...
# Deschutes UGB Population Changes
...
# Deschutes County: Bend UGB Boundaries
...
```{r echo = FALSE, message = FALSE, warning = FALSE,out.width="750px", out.height="500px", dpi=300}
ugb_plots$gr_compare[[focus_county]]$`Bend`
```
...
# Deschutes UGBs: La Pine in Context

.pux-right[
```{r echo = FALSE, message = FALSE, warning = FALSE,out.width="800px", out.height="400px", dpi=300}
ugb_plots$hist_pop_house_plot[[focus_county]]$`La Pine`
```
]
...
# Deschutes County: La Pine UGB Boundaries
...
# La Pine UGB Pop Forecast

```{r echo = FALSE, message = FALSE, warning = FALSE,out.width="750px", out.height="500px", dpi=300}
ugb_plots$ind_ugb_plot[[focus_county]]$`La Pine`
```
...
```{r echo = FALSE, message = FALSE, warning = FALSE,out.width="750px", out.height="500px", dpi=300}
ugb_plots$gr_compare[[focus_county]]$`La Pine`
```
...
# Deschutes UGBs: Redmond in Context

.pux-right[
```{r echo = FALSE, message = FALSE, warning = FALSE,out.width="800px", out.height="400px", dpi=300}
ugb_plots$hist_pop_house_plot[[focus_county]]$`Redmond`
```
]
...
# Deschutes County: Redmond UGB Boundaries
...
# Redmond UGB Pop Forecast

```{r echo = FALSE, message = FALSE, warning = FALSE,out.width="750px", out.height="500px", dpi=300}
ugb_plots$ind_ugb_plot[[focus_county]]$`Redmond`
```
...
```{r echo = FALSE, message = FALSE, warning = FALSE,out.width="750px", out.height="500px", dpi=300}
ugb_plots$gr_compare[[focus_county]]$`Redmond`
```
...
# Deschutes UGBs: Sisters in Context

.pux-right[
```{r echo = FALSE, message = FALSE, warning = FALSE,out.width="800px", out.height="400px", dpi=300}
ugb_plots$hist_pop_house_plot[[focus_county]]$`Sisters`
```
]
...
# Deschutes County: Sisters UGB Boundaries
...
# Sisters UGB Pop Forecast

```{r echo = FALSE, message = FALSE, warning = FALSE,out.width="750px", out.height="500px", dpi=300}
ugb_plots$ind_ugb_plot[[focus_county]]$`Sisters`
```
...
```{r echo = FALSE, message = FALSE, warning = FALSE,out.width="750px", out.height="500px", dpi=300}
ugb_plots$gr_compare[[focus_county]]$`Sisters`
```
